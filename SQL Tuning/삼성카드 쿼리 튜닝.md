...
select t1.vdn_no,
       ...
       max(case when t1.server_id in ('SC-CTI-01', 'DC-CTI-01') then result else '' end) as cti_01_result,
       max(case when t1.server_id in ('SC-CTI-02', 'DC-CTI-02') then result else '' end) as cti_02_result
...
where t1.center_id = 'center_id_value'
...
and t1.monitoring_yn = case when 'monitoring_yn_value' is null or 'monitoring_yn_value' = '' then t1.monitoring_yn else 'monitoring_yn_value' end
group by t1.vdn_no
         ,t1.center_id
         ,rs001.cd_nm
         ,t1.server_id
